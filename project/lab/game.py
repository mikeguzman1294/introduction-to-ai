...
#            raise Exception("INVALID MOVEMENT ENEMY")
    ...
    def _get_reward(self):
        ...
        if self.round > self.round_limit:
            ...
            # Check if the game ended and give reward according victory or defeat
            if self.enemy_score == self.score and self.score >= self.cheeses/2:
                return 0
            # Reward if the enemy wins the game CHANGE
            elif self.enemy_score > self.cheeses/2:
                return -1 # -1               
            # Reward if the agent wins the game CHANGE
            elif self.score > self.cheeses/2:
                return 2 # 1             
            # Reward 1 in any case the agent eats a cheese and game is not over
            else:
                return 1
            ...
            if self.enemy_score > self.cheeses/2:
                return -1 # -1
            # Reward if the enemy eats but does not win the game
            else:
                return 0.
        # Reward nothing if no one eats
        else:
            return 0.
